...
int main(void) {
    ...
    for (;;) {
        ...
        int addr_size;
        size_t bytes = recvfrom(sfd, msg, sizeof(msg) - 1, 0, (struct sockaddr*) &client_addr, (socklen_t*)&addr_size);
        if (bytes == -1) {
        ...
        bytes = sendto(sfd, back, strlen(back), 0, (struct sockaddr*) &client_addr, addr_size);
